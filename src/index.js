...
const selectors =
	"[matches], [required], [unique], [subdomain], [length], [email]";
...
async function validate(action) {
	...
	if (params) {
		...
		if (!validateElements.length) {
			...
			if (element) validateElements = element.querySelectorAll(selectors);
		}
	} else {
	...
	for (let [validation, func] of validations) {
		...
		if (elements.length) {
			...
			if (failedEls) failedElements.push(failedEls);
		}
	}
	for (let element of validateElements) {
		let isValid = element.getAttribute("validation");
		if (isValid == "false") failedElements.push(element);
	}
